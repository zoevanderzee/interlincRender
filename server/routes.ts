...
// Mock imports for dependencies that are not provided in the context
// In a real environment, these would be imported from their respective files.
const trolleyApi = {
  isConfigured: () => false,
  createPayment: async (data: any) => ({ success: false, error: 'Trolley API not available', payment: null }),
  getPayment: async (paymentId: string) => ({ success: false, error: 'Trolley API not available', payment: null }),
  createCompanyProfile: async (data: any) => ({ success: false, error: 'Trolley API not available', profileId: null }),
  getFundingInstructions: async () => ({ accountName: 'Trolley Bank', accountNumber: '12345678', sortCode: '11-22-33' })
};
...
const trolleySubmerchantService = {
  validateSubmerchantData: (data: any) => ({ valid: true, errors: [] }),
  createSubmerchantAccount: async (data: any) => ({ success: false, error: 'Trolley service not available', submerchantId: null, status: null }),
  checkBudgetAvailability: (cap: number, used: number, req: number) => false,
  createSubmerchantPayment: async (data: any) => ({ success: false, error: 'Trolley service not available', paymentId: null, batchId: null, status: null }),
  getFundingInstructions: async () => ({ accountName: 'Trolley Bank', accountNumber: '12345678', sortCode: '11-22-33' })
};
const trolleySdk = {
  createSubmerchant: async (data: any) => ({ merchant: { id: 'acct_123', accessKey: 'ak_test_123', secretKey: 'sk_test_123' }, status: 'pending_verification' })
};

// Mock implementations for Trolley routes if they are not defined elsewhere
const trolleyRoutes = (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ };
const trolleyTestRoutes = (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ };
const connectRoutes = (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ };
const plaidRoutes = (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ };
const adminRoutes = { default: (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ } };
const testV2Routes = { default: (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ } };
const trolleyContractorRoutes = { registerTrolleyContractorRoutes: (app: Express, apiRouter: string, requireAuth: any) => { /* no-op */ } };
...
<body>
  ...
  app.patch(`${apiRouter}/milestones/:id`, requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (userRole === 'contractor') {
        ...
        if (!hasAccess) {
          // Check if contractor has work requests for this contract
  ...
  app.patch(`${apiRouter}/deliverables/:id`, requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const { deliverableDescription, deliverableFiles, submissionType } = req.body;
      ...
      console.log(`Updating deliverable ${deliverableId} for user ${userId} with role ${userRole}`);

      // SECURITY: Verify user has access to update this deliverable
      const existingDeliverable = await storage.getMilestone(deliverableId); // Use same storage method
      if (!existingDeliverable) {
        console.log(`Deliverable ${deliverableId} not found`);
        return res.status(404).json({ message: "Deliverable not found" });
      }

      console.log(`Found deliverable ${deliverableId} for contract ${existingDeliverable.contractId}`);
      ...
      if (!contract) {
        console.log(`Contract ${existingDeliverable.contractId} not found for deliverable ${deliverableId}, checking for work request association`);
        ...
        try {
          ...
          if (relatedWorkRequest) {
            console.log(`Found related work request ${relatedWorkRequest.id} for deliverable ${deliverableId}`);
      ...
      if (userRole === 'contractor') {
        ...
        if (!hasAccess) {
          console.log(`Contractor ${userId} has no access to deliverable ${deliverableId}`);
      ...
      // Set submittedAt timestamp when status changes to completed
      if (req.body.status === 'completed' && !existingDeliverable.submittedAt) {
        req.body.submittedAt = new Date().toISOString();
      }

      // Update the deliverable using the data from req.body
      const updatedDeliverable = await storage.updateMilestone(deliverableId, req.body); // Use same storage method

      if (!updatedDeliverable) {
        return res.status(404).json({ message: "Deliverable not found after update" });
      }

      console.log(`Successfully updated deliverable ${deliverableId}`);

      // Create notification when contractor submits work (marks deliverable as completed)
      if (req.body.status === 'completed' && userRole === 'contractor') {
        try {
          if (contract) {
            await notificationService.createWorkSubmission(
              contract.businessId,
  ...
  */
  ...
  // Users should ONLY create Trolley accounts through the widget flow
  //
...
function registerTrolleySubmerchantRoutes(app: Express, apiRouter: string, requireAuth: any): void {
