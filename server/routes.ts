...
<body>
  ...
  app.get(`${apiRouter}/contractors/:id`, requireAuth, requireActiveSubscription, async (req: Request, res: Response) => {
    try {
      const contractorId = parseInt(req.params.id);
      ...
      if (!userId) {
        console.log("No user ID found when accessing individual contractor");
        return res.status(401).json({ message: "Authentication required" });
      }

      console.log(`Getting contractor ${contractorId} for business user ${userId}`);

      // Get the contractor
      const contractor = await storage.getUser(contractorId);
      if (!contractor) {
        console.log(`Contractor ${contractorId} not found in database`);
        return res.status(404).json({ message: "Contractor not found" });
      }

      console.log(`Found contractor: ${contractor.username} (${contractor.email})`);

      // Security: Only allow access to contractors that are connected to this business
      const user = await storage.getUser(userId);
      if (!user || user.role !== 'business') {
        console.log(`User ${userId} is not a business user`);
        return res.status(403).json({ message: "Only business users can access contractor details" });
      }

      // Check if this contractor is connected to the business
      const contractorsWithContracts = await storage.getContractorsByBusinessId(userId);
      let contractorsByConnections: any[] = [];
      ...
      try {
        ...
        console.log(`Found ${connections.length} accepted connections for business ${userId}`);

        for (const connection of connections) {
          if (connection.contractorId) {
            const connectedContractor = await storage.getUser(connection.contractorId);
            if (connectedContractor && connectedContractor.role === 'contractor') {
              contractorsByConnections.push(connectedContractor);
            }
          }
        }
      } catch (error) {
      ...
      // Check if contractor is in either list
      const allConnectedContractors = [...contractorsWithContracts, ...contractorsByConnections];
      const isConnected = allConnectedContractors.some(c => c.id === contractorId);

      console.log(`Contractor ${contractorId} connected to business ${userId}: ${isConnected}`);

      if (!isConnected) {
        console.log(`Contractor ${contractorId} not connected to business ${userId}`);
        return res.status(403).json({ message: "Contractor not connected to your business" });
      }
      ...
      try {
        const connectData = await storage.getConnectForUser(contractorId);

        if (connectData) {
          console.log(`Found Connect data for contractor ${contractorId}: ${connectData.accountId}`);
          // Get real-time Connect status from Stripe
          try {
            const account = await stripe.accounts.retrieve(connectData.accountId);
            connectAccountData = {
            ...
            console.log(`Stripe account data retrieved for ${contractorId}: ${account.country}/${account.default_currency}`);
          } catch (stripeError) {
            console.error(`Error getting Stripe account for contractor ${contractorId}:`, stripeError);
            connectAccountData = {
              country: connectData.country,
              defaultCurrency: connectData.defaultCurrency,
              accountId: connectData.accountId,
          ...
          console.log(`No Connect data found for contractor ${contractorId}`);
        }
      } catch (error) {
        console.error(`Error getting Connect data for contractor ${contractorId}:`, error);
      }

      // Return contractor with enhanced data
      const enhancedContractor = {
        ...contractor,
        connectAccountData
      };

      console.log(`Returning contractor data for ${contractorId}`);
      res.json(enhancedContractor);
    } catch (error) {
      console.error("Error fetching individual contractor:", error);
