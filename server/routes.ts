...
function registerTrolleySubmerchantRoutes(app: Express, requireAuth: any): void {
  ...
  app.get(`${apiRouter}/connect/accounts/:accountId`, requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const accountDetails = await stripeService.getConnectAccountDetails(accountId);
      
      res.json({
        success: true,
        account: accountDetails,
        onboarding_complete: accountDetails.charges_enabled && accountDetails.details_submitted,
      });
    } catch (error: any) {
  ...
  app.post(`${apiRouter}/connect/accounts/:accountId/account-links`, requireAuth, async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      ...
      const accountLink = await stripeService.createAccountLink(accountId, type);
  ...
  app.post(`${apiRouter}/connect/accounts/:accountId/products`, requireAuth, async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      ...
      const product = await stripeService.createConnectProduct(
        accountId, 
        name, 
        description, 
        priceInCents, 
  ...
  app.get(`${apiRouter}/connect/accounts/:accountId/products`, async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      ...
      const products = await stripeService.getConnectProducts(accountId);
      ...
      console.error('Error fetching Connect products:', error);
      res.status(500).json({ 
  ...
  app.post(`${apiRouter}/connect/accounts/:accountId/checkout`, async (req: Request, res: Response) => {
    try {
      const { accountId } = req.params;
      const { line_items, application_fee_amount = 50 } = req.body; // Default 50 cent platform fee
      ...
      if (!line_items || !Array.isArray(line_items)) {
        return res.status(400).json({ 
          error: 'Missing required field: line_items (array)' 
        });
      }

      const baseUrl = process.env.FRONTEND_URL || 'http://0.0.0.0:5000';
      const successUrl = `${baseUrl}/storefront/${accountId}/success?session_id={CHECKOUT_SESSION_ID}`;
      const cancelUrl = `${baseUrl}/storefront/${accountId}/cancel`;

      const session = await stripeService.createConnectCheckoutSession(
        accountId,
        line_items,
