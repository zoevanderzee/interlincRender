...
- July 28, 2025: **CRITICAL REGISTRATION BUG FIXED - AUTOMATIC TROLLEY ACCOUNT CREATION REMOVED** - Discovered and eliminated dangerous bug where `/api/trolley/recipient` endpoint was automatically creating Trolley accounts without user consent during contractor registration. This was the root cause of "Email already exists" widget errors. Removed automatic account creation entirely - users now only create Trolley accounts through explicit widget interaction. Fixed privacy violation where external accounts were created without user knowledge.
