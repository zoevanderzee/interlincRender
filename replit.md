...
**PROJECT MANAGEMENT ENHANCEMENTS (October 9, 2025)**: Significantly improved project creation experience and database integrity. DATABASE CLEANUP: Deleted 5 duplicate empty "test" projects, renamed 3 remaining active projects to meaningful names (Client Website Redesign, Mobile App Development, Marketing Campaign). SCHEMA ENHANCEMENT: Added deadline column to projects table for better project timeline tracking. IMPROVED UI/UX: Redesigned new project form with GBP currency formatting (£ instead of $), added deadline picker, enhanced layout with info card explaining multi-contractor project capabilities, added clearer placeholder text and helper instructions. BACKEND UPDATES: Updated POST /api/projects endpoint to accept deadline field, ensured proper Date conversion in storage layer. Projects Overview tab correctly filters out Quick Tasks to show only multi-contractor projects, Tasks tab exclusively displays Quick Tasks work requests - proper separation maintained.
